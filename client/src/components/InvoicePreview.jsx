...
export default function InvoicePreviewPage({ invoiceData, invoice }) {
  const accentColor = invoice?.template?.design?.accentColor || "#3B82F6";
  const letterheadUrl = invoice?.template?.design?.letterheadUrl || "";
  const termsText =
    invoiceData?.termsText || invoice?.template?.design?.termsText || "";
  const bottomLayerUrl = invoice?.template?.design?.bottomLayerUrl || "";
  ...
  return (
    <div className="p-4">
      {invoiceData ? (
        ...
        <p className="text-gray-500 dark:text-gray-300">Loading preview...</p>
      )}
    </div>
  );
}
